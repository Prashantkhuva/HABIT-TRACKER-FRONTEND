...
const Signup = () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (loginResponse.data) {
          ...
          dispatch(authLogin({ userData: { _id: user._id, email: user.email, username: user.username } }));
  ...
  return (
    <div className="relative min-h-screen bg-background text-text-primary overflow-hidden">
      <div className="max-sm:hidden absolute right-10 top-10 w-50 h-77.5 rounded-3xl overflow-hidden opacity-30 pointer-events-none">
        <img src="/authpage1.png" alt="" loading="lazy" className="w-full h-full object-cover scale-125 blur-[1px]" />
      </div>
      <div className="max-sm:hidden absolute left-10 bottom-10 w-42.5 h-42.5 rounded-full overflow-hidden opacity-40 pointer-events-none">
        <img src="/image.png" alt="" loading="lazy" className="w-full h-full object-cover scale-125 blur-[3px]" />
      </div>
        ...
        <div className="min-h-[60vh] flex items-center justify-center px-4 py-10 sm:py-0">
          ...
          >
            <div className="bg-surface/80 backdrop-blur-md border border-border-subtle/60 rounded-3xl p-8 shadow-xl">
              <h1 className="font-heading text-2xl font-bold mb-2 text-text-primary">create account</h1>
              <p className="text-sm text-text-muted mb-6">begin your ritual journey.</p>
              {error && <p className="text-danger text-sm mb-4">{error}</p>}
              <form onSubmit={handleSubmit(create)} className="space-y-4">
                <Input label="Username" placeholder="julian_vane" {...register("username", { required: true, minLength: 3 })} />
                <Input type="email" label="Email" placeholder="hello@habitflow.com" {...register("email", { required: true })} />
                <Input type="password" label="Password" placeholder="••••••••" {...register("password", { required: true, minLength: 6 })} />
                <Input type="password" label="Confirm Password" placeholder="••••••••" {...register("confirmPassword", { required: true, validate: (v) => v === password || "Passwords do not match" })} />
              ...
              <p className="text-center text-sm mt-6 text-text-muted">
                ...
                <Link href="/signin" className="font-medium text-text-primary hover:opacity-70 transition-opacity">Sign in</Link>
              </p>
            </div>
          </motion.div>
        </div>
